function clickOpenTooltip() {
    ...
    let voContent = $(div).attr('aria-label');
...
function clickOpenModal() {
    ...
    let closeButton = `<button class='close-modal' aria-label='` + closeWord + `'><svg viewBox="0 0 30 30" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
							<g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
								<g id="Group-12" transform="translate(0.000000, 0.000000)" stroke="#fff">
									<line x1="10.6966991" y1="10.6966991" x2="21.3033009" y2="21.3033009" id="Line-Copy" stroke-width="1.5" stroke-linecap="round"></line>
									<line x1="21.3033009" y1="10.6966991" x2="10.6966991" y2="21.3033009" id="Line-Copy-2" stroke-width="1.5" stroke-linecap="round"></line>
								</g>
							</g>
						</svg></button>"`
    let scrollGradient = '<div class="scroll-gradient"></div>'
...
function hotspot(hotSpotDiv, args) {
    ...
    if(args[5] == 'pano-hs-image') {
        ...
        $(hotSpotDiv).attr({
            'role': 'button',
            'aria-label': args[2] + args[6] + args[1],
            'index': args[4]
        })
        ...
        if(parseInt(innerDiv.style.width) < 200) {
            
        } else if (parseInt(innerDiv.style.width) < 345) {
            let newWidth = ($(innerDiv).width() + 100);
            innerDiv.style.width = newWidth + 'px';
            
        } else {
            let newWidth = ($(innerDiv).width() + 300);
            if (newWidth > 520){
                newWidth = 520;
            }
            innerDiv.style.width = newWidth + 'px';
...
$(document).ready(function () {
    ...
    $('.panorama').each(function () {
        ...
        for (var i = 0; i < hotspots.length; i++) {
            ...
            let strippedHtmlContent = $(hotspots[i]).text();
            ...
            if(contentType == 'pano-hs-text') {
                var newData = {
                    ...
                    "createTooltipArgs": [createTooltipArgs, voCount, hotspotLabel, hotspotStyle, (i+1), contentType, strippedHtmlContent],
        ...
        let newViewer = pannellum.viewer(panoId, {
    ...
    $('.nexthotspot').on('click', function (e) {
        ...
        if($(currentHotspotInView).hasClass('pano-hs-text') && $(currentHotspotInView).find('.hs-inner-wrap').height() > 100){
            nextPitch = parseInt(nextPitch) + (parseInt($(currentHotspotInView).find('.hs-inner-wrap').height()) / 20);
        }
